"""baseline: create SECRET relational schema.

Revision ID: 0001_baseline
Revises:
Create Date: 2026-08-28

Embeds the canonical Phase 1 DDL (see backend/sql/schema.sql) so PostgreSQL can
be brought to a known state with `alembic upgrade head`. This is the migration
root; later phases append model-driven changes on top.
"""
...
# Matches backend/sql/schema.sql (single source of truth) for Phase 2.
_SCHEMA_SQL = r"""
CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE TYPE user_role   AS ENUM ('admin', 'analyst', 'investigator', 'viewer');
CREATE TYPE user_status AS ENUM ('active', 'disabled');
CREATE TYPE entity_type AS ENUM ('PERSON','ORGANIZATION','PHONE','VEHICLE','LOCATION','ACCOUNT');
...
CREATE TYPE evidence_type AS ENUM ('DOCUMENT','PHOTO','VIDEO','AUDIO','RECORD','OTHER');
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        DROP TABLE IF EXISTS fir_profiles;
        ...
        DROP TYPE IF EXISTS fir_status;
